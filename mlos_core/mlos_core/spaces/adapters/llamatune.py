...
class LlamaTuneAdapter(
    ...
    def __init__(
        ...
        *,
        ...
        max_unique_values_per_param: Optional[
            int
        ...
        if num_low_dims >= len(orig_parameter_space):
            raise ValueError(
                "Number of target config space dimensions should be less than those of original config space."
            )
        ...
        self._suggested_configs: Dict[
            ConfigSpace.Configuration, ConfigSpace.Configuration
        ] = {}
    ...
    def inverse_transform(self, configurations: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, config in configurations.astype("O").iterrows():
            ...
            if target_config is None:
                ...
                if (
                    configuration
                    == self.orig_parameter_space.get_default_configuration()
                ):
                    # Default configuration should always be registerable.
                    pass
                elif not self._use_approximate_reverse_mapping:
                    raise ValueError(
                        "The configuration was not suggested by the optimizer. "
    ...
    def transform(self, configuration: pd.DataFrame) -> pd.DataFrame:
        ...
        orig_configuration = normalize_config(
            self.orig_parameter_space, orig_values_dict
        )
    ...
    def _construct_low_dim_space(
        ...
        if max_unique_values_per_param is None:
            hyperparameters = [
                ConfigSpace.UniformFloatHyperparameter(
                    name=f"dim_{idx}", lower=-1, upper=1
                )
        ...
        config_space = ConfigSpace.ConfigurationSpace(
            name=self.orig_parameter_space.name
        )
        config_space.random = (
            self._random_state
    ...
    def _transform(self, configuration: dict) -> dict:
        ...
        original_config_values = self._config_scaler.transform(
            [original_config_values]
        )[0]
        ...
        for param, value in special_param_values_dict.items():
            ...
            if isinstance(value, int):
                ...
                if all(isinstance(t, int) for t in value):
                    ...
                    tuple_list = [
                        (v, self.DEFAULT_SPECIAL_PARAM_VALUE_BIASING_PERCENTAGE)
                        for v in value
                    ]
                elif all(
                    isinstance(t, tuple) and [type(v) for v in t] == [int, float]
                    for t in value
                ):
                    ...
                    raise ValueError(
                        error_prefix
                        + f"Invalid format in value list for parameter '{param}'. "
                        f"Special value list should contain either integers, or (special value, biasing %) tuples."
                    )
            else:
                raise ValueError(
                    error_prefix
                    + f"Invalid format for parameter '{param}'. Dict value should be "
                    "an int, a (int, float) tuple, a list of integers, or a list of (int, float) tuples."
                )

            # Are user-specified special values valid?
            if not all(
                hyperparameter.lower <= v <= hyperparameter.upper for v, _ in tuple_list
            ):
                raise ValueError(
                    error_prefix
                    + f"One (or more) special values are outside of parameter '{param}' value domain."
                )
            # Are user-provided special values unique?
            if len(set(v for v, _ in tuple_list)) != len(tuple_list):
                raise ValueError(
                    error_prefix
                    + f"One (or more) special values are defined more than once for parameter '{param}'."
                )
    ...
    def _try_generate_approx_inverse_mapping(self) -> None:
        ...
        warn(
            "Trying to register a configuration that was not previously suggested by the optimizer. "
            + "This inverse configuration transformation is typically not supported. "
            + "However, we will try to register this configuration using an *experimental* method.",
            UserWarning,
        )
