...
class BaseOptimizer(metaclass=ABCMeta):
    ...
    def register(
        ...
        if type(self._optimization_targets) is str:
            assert (
                self._optimization_targets in scores.columns
    ...
    def get_observations(
        ...
        configs = pd.concat(
            [config for config, _, _ in self._observations]
        ).reset_index(drop=True)
        scores = pd.concat([score for _, score, _ in self._observations]).reset_index(
            drop=True
        )
        contexts = pd.concat(
            [
